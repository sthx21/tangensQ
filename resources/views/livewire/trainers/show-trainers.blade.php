        ...
        <div class="col-md-10 col-12 w-full">
            <div class="tableTopMenu">
                ...
                <div class="tableTopRight">
                    <x-button-link :link="'/trainers/create'" :class="'btn-info'" :title="'New Company'">{{trans('trainers.buttons.new')}}</x-button-link>
            ...
            <x-index-table>
                <table class="table table-striped table-sm data-table table-hover" id="companies_table">
                    ...
                    <tbody>
                    <div>
                        @foreach ($tagFilteredTrainers as $trainer)
                            <tr>
                                <td>{{$trainer->id}}</td>
                                ...
                                <td>{{$trainer->email}}</td>
                                <td></td>
                                <td style="display: flex; justify-content: right"><button class="editButton" wire:click.prevent="editTrainer({{$trainer}})" wire:key="edit.{{$trainer->id}}">Bearbeiten</button>
                                    <button class="detailsButton" wire:click.prevent="showTrainer({{$trainer}})" wire:key="show.{{$trainer->id}}">Details</button>
                                </td>
                            </tr>
                        @endforeach
                    </div>
